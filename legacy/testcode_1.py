...
def generate_and_solve_system_with_plot(manifold_name, L=1, num_points=1000, min_images_per_point=10, k_values=None, num_points_to_use=None, resolution=100):
    ...
    if pairing_matrices is not None and len(selected_points) > 0:
        ...
        if k_values is None:
            k_values = np.linspace(8.0, 10.0, resolution)  # Adjust the range and resolution as needed
...
# Run the function for varying L values
#for i in range(6, 10,1):
    #generate_and_solve_system_with_plot('m003(-2,3)', L=6, num_points=1000, num_points_to_use=i)
    
generate_and_solve_system_with_plot('m003(-2,3)', L=14, num_points=1000, num_points_to_use=30, resolution=50)